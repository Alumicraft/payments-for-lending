...
class TestMapBlockContent(unittest.TestCase):
    ...
    def test_workspace_css_keeps_full_bleed_scoped_to_map(self):
        css = (ROOT / "dcr/public/css/workspace_fullwidth.css").read_text()

        self.assertIn("Temporarily scoped to the Map workspace only", css)
        self.assertIn('body.dcr-workspace-fullbleed[data-route="Workspaces/Map"]', css)
        self.assertIn('body[data-route="Workspaces/Map"] .container.page-body', css)
        self.assertIn('body[data-route="Workspaces/Map"]:has(.workspace-body) .container.page-body', css)
        ...
        self.assertNotIn("body.dcr-workspace-fullbleed .page-body", css)
        self.assertNotIn("padding-left: var(--dcr-workspace-content-padding, 20px) !important", css)
        self.assertNotIn("padding-right: var(--dcr-workspace-content-padding, 20px) !important", css)
...
class TestPackagingConfig(unittest.TestCase):
    ...
    def test_pyproject_packages_desk_assets_for_frappe_cloud(self):
        ...
        self.assertIn('DCR_ASSET_VERSION = "20260508-3"', hooks)
